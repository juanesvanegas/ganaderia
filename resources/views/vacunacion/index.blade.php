            ...
            <div class="col-md-12" >
                <div class="card card-user">
                    ...
                    <div class="card-body">
                        ...
                        <a class="btn btn-success" href="{{route('create.vacunacion')}}">Crear aplicación</a>
                        <a class="btn btn-success"  href="{{route('welcome')}}" style="position: relative;  left:1280px ; " >Inicio</a> 
                        <p class="card-text">
                            <div class="author table-responsive">
                                <div class="card-body">
                                    <table class="table table-bordered table-striped" id="vacunacion">
                                        <thead class="table-secondary">
                                            <tr style="text-align: center">
                                   
                                                <th scope="col">Animal</th>
                                                <th scope="col">Medicamento aplicado</th>
                                        ...
                                        <tbody>
                                            @foreach ($vacunacion as $info)
                                                <tr style="text-align: center">
                                                    <td>{{$info->animals->identificador}}</td>
                                                    <td>{{$info->medicamentos->nombre_medic}}</td>
                                                    <td>{{$info->medio_admin}}</td>
                                                    <td>{{$info->cantidad_usada}}</td>
                                                    <td>{{$info->unidad_medi}}</td>
                                                    <td>{{$info->fecha_realizacion}}</td>
                                                        ...
                                                        <div class="dropdown">
                                                            <a class="btn btn-sm btn-icon-only text-dark" style="font-size: 20px" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                                ...
                                                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                                                   ...
                                                                   <a href="{{ route('vacunacion.edit', $info->id) }}" class="btn btn-outline-warning"><i class="material-icons">edit</i></a>

                                                                    <form action="{{route('destroy.vacunacion',$info->id)}}" class="form-eliminar" method="POST" style="display:inline-block">
                                                                        @csrf
                                                                        @method('DELETE')
